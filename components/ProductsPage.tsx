...
const ProductsPage: React.FC = () => {
  const [allProducts, setAllProducts] = useState<ProductItem[]>(content.products_page.items as ProductItem[]);

  useEffect(() => {
    const load = async () => {
      const dbProducts = await getProducts();
      if (dbProducts.length === 0) return;
      const staticProducts = content.products_page.items as ProductItem[];
      const staticIds = new Set(staticProducts.map(p => p.id));
      const merged = staticProducts.map(sp => {
        const override = dbProducts.find(dp => dp.id === sp.id);
        return override ?? sp;
      });
      const purelyNew = dbProducts.filter(dp => !staticIds.has(dp.id));
      setAllProducts([...merged, ...purelyNew].filter(p => p.id !== 'PROD_05'));
    };
    load();
  }, []);
                     ...
                     <div className="relative z-10 flex flex-col md:flex-row gap-4 md:items-center justify-between">
                        ...
                        <div className="md:border-l-2 md:border-black/10 md:pl-6 flex flex-col items-center justify-center shrink-0 gap-3">
                           ...
                           {item.link ? (
                              ...
                              >
                                 {item.id === 'PROD_04' ? 'GARANTIR ACESSO' : (item.price || 'R$897,00')} <ChevronRight size={18} />
                              </a>
                           ) : (

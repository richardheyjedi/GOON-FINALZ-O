...
import { Check, Zap, Crown, Infinity, ShieldAlert, Lock, AlertTriangle, Search } from 'lucide-react';
import { content, Mentor } from '../content';
import MentorModal from './MentorModal';
import { getMentors } from '../lib/supabase';

const mergeMentors = (): Mentor[] => {
   const staticMentors = content.mentorship.mentors as Mentor[];
   try {
      const stored = localStorage.getItem('goon_custom_mentors');
      if (stored) {
         const custom = JSON.parse(stored) as Mentor[];
         const merged = staticMentors.map(sm => {
            const customOverride = custom.find(cm => cm.id === sm.id);
            return customOverride ? customOverride : sm;
         });
         const staticIds = new Set(staticMentors.map(m => m.id));
         const purelyCustom = custom.filter(cm => !staticIds.has(cm.id));
         return [...merged, ...purelyCustom];
      }
   } catch {
      // silently fall back to static mentors
   }
...
const MentorshipPage: React.FC = () => {
   ...
   const [allMentors, setAllMentors] = useState<Mentor[]>(content.mentorship.mentors as Mentor[]);

   useEffect(() => {
      const load = async () => {
         const dbMentors = await getMentors();
         if (dbMentors.length === 0) return; // keep static if DB empty
         // Merge: static base, overridden by DB when IDs match
         const staticMentors = content.mentorship.mentors as Mentor[];
         const staticIds = new Set(staticMentors.map(m => m.id));
         const merged = staticMentors.map(sm => {
         ...
         const purelyNew = dbMentors.filter(dm => !staticIds.has(dm.id));
         setAllMentors([...merged, ...purelyNew]);
      };
      load();
   }, []);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {allMentors.map((mentor) => (
                  <div 
                    ...
                    className="group flex flex-col bg-white border-4 border-black hover:-translate-y-2 hover:shadow-[8px_8px_0_0_#000] transition-all duration-300 cursor-pointer relative"
                  >
                     {/* Hover Overlay Indicator */}
                     <div className="absolute inset-0 z-30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/5 pointer-events-none">
                        <div className="bg-blue-600 text-white px-4 py-2 font-mono text-xs font-black uppercase border-2 border-black shadow-hard flex items-center gap-2">
                           <Search size={14} />
                           View_Profile
                        </div>
                     </div>
                     ...
                     <div className="relative aspect-[4/3] overflow-hidden border-b-4 border-black bg-gray-100">
                        ...
                        <div className="p-6 flex flex-col flex-grow relative overflow-hidden">
                           ...
                           <div className="relative z-10">
                              ...
                              <p className="font-mono text-xs leading-relaxed text-gray-700 mb-6 border-l-2 border-gray-200 pl-3 line-clamp-3">
                                 {mentor.bio}
                              </p>

...
const Navbar: React.FC<NavbarProps> = ({ activeTab, onNavigate }) => {
  ...
  const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className="flex flex-col h-full bg-white text-black w-full">
      {/* Brand Header with GOON Logo - Enlarged and Highlighted */}
      <div className={`${isMobile ? 'h-56' : 'h-80'} bg-black text-white flex flex-col items-center justify-center border-b-4 border-black p-6 relative overflow-hidden group shrink-0`}>
        
        {/* Vintage Scanlines & Noise Background */}
        ...
        {/* CRT Screen Glow Effect Behind Logo */}
        ...
        <div className="relative z-10 w-40 h-40 md:w-56 md:h-56 flex items-center justify-center perspective-[1000px]">
           <img 
           ...
           {/* Glitch Overlay (Ghosting effect on hover) */}
           <img 
             src="https://richardhey.com.br/wp-content/uploads/2026/01/logo-goon-provisoria.png" 
             alt="" 
             className="absolute top-0 left-0 w-full h-full object-contain animate-spin-y opacity-0 group-hover:opacity-40 mix-blend-color-dodge transition-opacity duration-300 pointer-events-none"
             style={{ animationDelay: '0.1s', filter: 'blur(4px) brightness(1.5)' }}
           />
        </div>
        
        <div className="mt-4 text-[10px] font-mono tracking-[0.5em] text-white/50 opacity-80 uppercase border border-white/20 px-2 py-1 bg-black/50 backdrop-blur-sm">
          Est. {content.global.year}
        </div>
      </div>

      {/* Navigation Links */}
      <nav className="flex-1 overflow-y-auto bg-white flex flex-col custom-scrollbar">
        {(content.navbar as NavItem[]).map((item, index) => {
          ...
          return (
            <button
              ...
              className={`
                w-full text-left p-4 lg:p-5 border-b-2 border-black group transition-all duration-200 relative overflow-hidden shrink-0
            ...
            >
              ...
              <div className="relative z-10 flex items-center justify-between h-full">
                <div className="flex items-center gap-4">
                  {/* Index Number */}
                  <span className={`font-mono text-xs font-bold opacity-50 w-6 ${isActive ? 'text-white' : 'text-black group-hover:text-white'}`}>
                    0{index + 1}
                  </span>
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-bold uppercase tracking-wider text-sm ${isActive ? 'text-white' : 'text-black group-hover:text-white'}`}>
                        {item.name}
                      </span>
                    </div>
                    {/* Small Description */}
                    <span className={`text-[10px] font-mono uppercase tracking-widest mt-0.5 ml-7 ${isActive ? 'text-gray-400' : 'text-gray-500 group-hover:text-gray-400'}`}>
                      {item.desc}
                    </span>
                  </div>
                </div>
      ...
      <div className="p-6 bg-retro-bg border-t-4 border-black shrink-0">
        <div className="border-2 border-black bg-white p-3 shadow-hard-sm hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all cursor-pointer group">
           <div className="flex justify-between items-end">
             ...
             <div className="text-[10px] font-mono font-bold group-hover:underline">
               100%
             </div>
           </div>
        </div>
    ...
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 w-full bg-white border-b-4 border-black z-50 px-4 py-3 flex justify-between items-center h-[70px]">
        ...
        <button 
          ...
          className="p-2 border-2 border-black shadow-hard-sm active:shadow-none active:translate-y-1 bg-retro-bg hover:bg-black hover:text-white transition-colors"
        >
          <Menu size={24} />
        </button>
      </div>
      ...
      {isOpen && (
        <div className="md:hidden fixed inset-0 z-[60] flex">
          ...
          <div className="relative w-[85%] max-w-[320px] h-full border-r-4 border-black shadow-2xl animate-slide-in-left bg-white flex flex-col">
            <button 
              onClick={() => setIsOpen(false)}
              className="absolute top-2 right-2 z-50 p-2 bg-white border-2 border-black text-black hover:bg-black hover:text-white transition-colors shadow-sm"
            >
              <X size={20} />
            </button>

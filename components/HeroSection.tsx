...
import { ArrowDown, Infinity, Crown, Play, Terminal, Database, ArrowRight, Maximize2, Barcode, CheckCircle2, Instagram, Youtube, ScanFace, Signal, Quote, MessageSquare, X } from 'lucide-react';
...
const mergeTestimonials = (): Testimonial[] => {
   const staticTestimonials = content.hero.testimonials as Testimonial[];
   try {
      const stored = localStorage.getItem('goon_custom_results');
      if (stored) {
         const custom = JSON.parse(stored) as Testimonial[];
         const merged = staticTestimonials.map(st => {
            const customOverride = custom.find(ct => ct.id === st.id);
            return customOverride ? customOverride : st;
         });
         const staticIds = new Set(staticTestimonials.map(t => t.id));
         const purelyCustom = custom.filter(ct => !staticIds.has(ct.id));
         return [...merged, ...purelyCustom];
      }
   } catch {
...
const HeroSection: React.FC<HeroSectionProps> = ({ onNavigate }) => {
   ...
   const [selectedResult, setSelectedResult] = useState<Testimonial | null>(null);
   const [allTestimonials, setAllTestimonials] = useState<Testimonial[]>(content.hero.testimonials as Testimonial[]);

   // Fetch from Supabase on mount, merge with static fallback
   useEffect(() => {
      const load = async () => {
         const dbResults = await getResults();
         if (dbResults.length === 0) return;
         const staticList = content.hero.testimonials as Testimonial[];
         const staticIds = new Set(staticList.map(t => t.id));
         const merged = staticList.map(st => dbResults.find(r => r.id === st.id) ?? st);
         const purelyNew = dbResults.filter(r => !staticIds.has(r.id));
         setAllTestimonials([...merged, ...purelyNew]);
      };
      load();
   }, []);
   ...
   useGSAP(() => {
      ...
      gsap.from(".gsap-testimonial-card", {
         scrollTrigger: {
            trigger: ".gsap-testimonials-section",
            start: "top 75%",
         },
         x: -30,
         opacity: 0,
   ...
   return (
      <div ref={containerRef} className="w-full bg-retro-bg overflow-x-hidden">
                           ...
                           <div className="flex items-start gap-4">
                              ...
                              <div className="flex-grow">
                                 ...
                                 <p className="text-xs sm:text-sm font-mono text-gray-600 font-bold leading-tight mb-4">
                                    {content.hero.modules.infinity.description}
                                 </p>
                              </div>
                           </div>
               ...
               <div className="lg:col-span-8 bg-white relative">
                  ...
                  <div className="p-4 md:p-8 bg-[url('https://www.transparenttextures.com/patterns/graphy.png')] bg-gray-50 relative">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-6 relative z-10">
                        {allTestimonials.slice(0, showAllCases ? undefined : 6).map((t, i) => (
                           <div 
                              key={i} 
                              onClick={() => setSelectedResult(t)}
                              className="gsap-video-card group relative flex flex-col h-full bg-white border-4 border-black shadow-hard hover:shadow-[8px_8px_0_0_#ccff00] transition-all duration-300 cursor-pointer hover:-translate-y-1"
                           >
                              ...
                              <div className="relative aspect-video overflow-hidden border-b-4 border-black bg-black">
                                 ...
                                 {t.video && (
                                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                       <div className="bg-[#ccff00] text-black p-4 border-4 border-black shadow-hard transform scale-75 group-hover:scale-100 transition-transform duration-300">
                              ...
                              <div className="p-4 flex flex-col justify-between flex-grow bg-white relative min-h-[160px]">
                                 <div className="flex justify-between items-start mb-2">
                                    <div className="flex flex-col pr-2">
                                       ...
                                       <h3 className="text-black font-black font-sans text-lg uppercase leading-tight">{t.subject}</h3>
                                    </div>
                                 ...
                                 <div className="mt-4 pt-3 border-t-2 border-dashed border-gray-300 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3">
                                    <span className="font-mono text-[10px] uppercase bg-gray-200 px-2 py-0.5 text-gray-700 font-bold shrink-0">{t.role}</span>
                                    <div className="flex flex-col items-start sm:items-end w-full">
                                       ...
                                       <div className="font-pixel text-[10px] md:text-xs font-bold uppercase bg-black text-[#ccff00] px-2 py-2 transform -rotate-1 shadow-sm w-full text-center sm:text-right leading-tight">
                                          {t.impact}
                                       </div>
                                    </div>
                                 </div>
         ...
         {/* === SYSTEM_FEEDBACK: TEXT TESTIMONIALS === */}
         <section className="relative py-20 border-t-4 border-black bg-white overflow-hidden gsap-testimonials-section">
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-12 relative z-10">
               <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
                  <div>
                     <div className="flex items-center gap-2 mb-4 bg-black text-[#ccff00] px-3 py-1 self-start w-fit">
                        <MessageSquare size={14} />
                        <span className="font-mono text-[10px] font-bold uppercase tracking-widest">COMMS_LOG_V5.1</span>
                     </div>
                     <h2 className="text-4xl md:text-6xl font-black uppercase font-pixel tracking-tighter leading-none mb-2">
                        RELATÓRIOS_<span className="text-gray-400 font-sans">DE_OPERAÇÃO</span>
                     </h2>
                     <p className="font-mono text-sm text-gray-500 uppercase font-bold tracking-tight">
                        Depoimentos reais de quem implementou o protocolo.
                     </p>
                  </div>
                  <div className="hidden md:flex flex-col items-end">
                     <span className="font-mono text-[10px] text-gray-400 font-bold uppercase mb-1">DATA_INTEGRITY: 100%</span>
                     <div className="w-48 h-2 bg-gray-100 border border-black relative">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {(content.hero as any).textTestimonials.map((item: any, i: number) => (
                     <div key={i} className="gsap-testimonial-card group relative">
                        <div className="absolute inset-0 bg-black translate-x-1.5 translate-y-1.5 group-hover:translate-x-2.5 group-hover:translate-y-2.5 transition-transform duration-300"></div>
                        <div className="relative bg-white border-2 border-black p-6 md:p-8 flex flex-col h-full overflow-hidden">
                           
                           {/* Card Top / Header */}
                           <div className="flex items-center gap-4 mb-6 relative">
                              <div className="relative shrink-0">
                                 <div className="w-16 h-16 border-2 border-black overflow-hidden relative grayscale hover:grayscale-0 transition-all duration-500 bg-gray-100">
                                    <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                                    <div className="absolute inset-0 border-t border-black/10 pointer-events-none"></div>
                                 </div>
                                 <div className="absolute -bottom-1 -right-1 bg-[#ccff00] border border-black p-1">
                                    <CheckCircle2 size={10} className="text-black" />
                                 </div>
                              </div>
                              <div className="flex flex-col">
                                 <h4 className="font-black text-lg leading-tight uppercase tracking-tight">{item.name}</h4>
                                 <span className="font-mono text-[9px] font-bold text-gray-500 uppercase tracking-widest">{item.role}</span>
                              </div>
                           </div>

                           {/* Quote / Content */}
                           <div className="relative flex-grow">
                              <Quote className="absolute -top-2 -left-2 text-gray-100 group-hover:text-[#ccff00]/10 transition-colors w-12 h-12 -z-0" />
                              <p className="relative z-10 font-mono text-sm leading-relaxed text-gray-800 font-medium italic">
                                 "{item.quote}"
                              </p>
                           </div>

                           {/* Card Footer / Technical Data */}
                           <div className="mt-8 pt-4 border-t border-dashed border-gray-300 flex justify-between items-center bg-gray-50 -mx-6 -mb-8 px-6 py-3">
                              <div className="flex gap-1.5">
                                 <div className="w-1 h-1 bg-black rounded-full animate-pulse"></div>
                                 <div className="w-1 h-1 bg-black rounded-full animate-pulse delay-75"></div>
                                 <div className="w-1 h-1 bg-black rounded-full animate-pulse delay-150"></div>
                              </div>
                              <span className="font-mono text-[9px] font-bold text-gray-400 uppercase tracking-widest">STAMP: VERIFIED_OPS</span>
                           </div>
                        </div>
                     </div>
                  ))}
         ...
         {selectedResult && (
            ...
            >
               <div 
                  className="w-full max-w-5xl bg-white border-4 border-black shadow-[20px_20px_0_0_#ccff00] relative flex flex-col pointer-events-auto overflow-hidden"
                  onClick={e => e.stopPropagation()}
               >
                  
                  {/* Header */}
                  <div className="bg-black text-[#ccff00] p-4 flex justify-between items-center border-b-4 border-black">
                     <div className="flex items-center gap-3">
                        <ScanFace size={20} />
                        <span className="font-mono text-xs font-bold uppercase tracking-[0.2em]">{selectedResult.id} // EVIDENCE_FILE</span>
                     </div>
                     <button onClick={() => setSelectedResult(null)} className="hover:rotate-90 transition-transform p-1">
                        <X size={24} />
                     </button>
                  </div>
                  ...
                  <div className="flex flex-col lg:flex-row h-full max-h-[85vh]">
                     ...
                     <div className="flex-1 bg-black relative flex items-center justify-center min-h-[300px] lg:min-h-0">
                        {(() => {
                           const videoUrl = (selectedResult.video || "").trim();
                           // Sanitize: only allow http(s) URLs
                           const isSafeUrl = videoUrl.startsWith('http://') || videoUrl.startsWith('https://');
                           const isYouTube = isSafeUrl && (videoUrl.includes('youtube.com') || videoUrl.includes('youtu.be'));
                           const isDirectVideo = isSafeUrl && (videoUrl.endsWith('.mp4') || videoUrl.endsWith('.webm') || videoUrl.endsWith('.mov'));
                           ...
                           if (isYouTube) {
                              ...
                              // If no video or just image, show thumbnail/image
                              return (
                                 <img 
                                    src={selectedResult.video || selectedResult.thumbnail} 
                                    alt={selectedResult.subject} 
                                    className="w-full h-full object-contain" 
                                 />
                              );
                           }
                     ...
                     {/* Content Sidebar */}
                     <div className="w-full lg:w-80 bg-white border-t-4 lg:border-t-0 lg:border-l-4 border-black p-6 flex flex-col justify-between overflow-y-auto">
                        <div>
                           <div className="mb-6">
                              <span className="font-mono text-[10px] font-black uppercase text-blue-600 bg-blue-50 px-2 py-0.5 border border-blue-200 inline-block mb-2">
                                 {selectedResult.role}
                              </span>
                              <h2 className="text-3xl font-black uppercase leading-none tracking-tighter mb-4">
                                 {selectedResult.subject}
                              </h2>
                           </div>

                           <div className="space-y-4">
                              <div className="p-3 bg-gray-50 border-2 border-black border-dashed">
                                 <span className="font-mono text-[9px] font-black uppercase text-gray-400 block mb-1">Impact_Report:</span>
                                 <p className="font-mono text-xs font-bold leading-relaxed">
                                    {selectedResult.impact}
                                 </p>
                              </div>
                              {selectedResult.impactLabel && (
                                 <div className="bg-[#ccff00] text-black p-2 border-2 border-black shadow-hard-sm">
                                    <span className="font-pixel text-[10px] font-bold uppercase">{selectedResult.impactLabel}</span>
                                 </div>
                              )}
                           </div>
                        </div>
                        ...
                        <button 
                           onClick={() => setSelectedResult(null)}
                           className="mt-8 w-full bg-black text-white py-4 font-black uppercase text-xs tracking-widest border-2 border-black hover:bg-[#ccff00] hover:text-black transition-all shadow-hard"
                        >
                           FECHAR_ARQUIVO
                        </button>
                     </div>
